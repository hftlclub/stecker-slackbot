...
Die kommunikation zwischen der Datenbank und dem Chatbot erfolgt auf dem gleichen Host, weshalb keine besonderen Fälle beachtet werden müssen. Der Kommunikationspfad zwischen dem Chatbot und Slack wird jedoch über ein Hochschulnetzwerk durchgeführt. In diesem sind in das Netzwerk des Clubs nur die Ports 80 und 443 freigegeben. Ausgehende Verbindungen sind durch ein doppeltes NAT beschränkt, was jedoch keine Einschränkung bedeutet, da der Slack-Service eine API bereitstellt und Standardports nutzt. Aus diesem Grund muss sich der Chatbot als Client mit Slack verbinden und darf keine Serveranwendung sein, welche ein Polling implementiert.
